...
Based on the literature discussed above, I pose three hypotheses at the state-level. It is worth noting that **H1a** and **H1b** are mutually exclusive.

* **H1a**: Following stated concerns of legislators around the country, I expect that more restrictive voting provisions were introduced and passed in 2021 in states where the COVI was lower in 2020.

* **H1b**: Following literature indicating that the pre-2021 difficulty of voting in a state was already indicative of how likely that state was to react to displays of nonwhite political power, I expect states with higher COVIs introduced and passed more restrictive legislation in 2021 than states with lower COVIs.

* **H1c**: Following theories of partisan advantage, I expect that more restrictive voting provisions were introduced in electorally-competitive states where Republicans had unified control over state government.
...
In Table \ref{tab:state}, I present the results of tests on my first set of hypotheses. Models 1--3 present the results of models where the dependent variable is the number of *introduced* restrictive provisions; Models 4--6, on the other hand, explore counts of passed provisions. By way of reminder, Hypotheses 1a and 1b are mutually exclusive; as such, they can be tested at the same time, and are included in Models 1, 3, 4, and 6.
...
These models uncover substantial support for the hypotheses arguing that race matters more than partisan control. The addition of partisan control variables to models controlling for race and COVI (that is, moving from Model 1 to Model 3, and Model 4 to Model 6) does not meaningfully alter the size of the cofficients on race and COVI (or their statistical significance levels), nor does their addition improve the R^2^ meaningfully (in fact, the adjusted R^2^ actually *decreases* with the addition of partisan controls when looking at passed provisions).

The same is not true of adding race and COVI covariates to the partisan control models (moving from Model 2 to 3, and 5 to 6). The coefficients on *Unified Republican Control* change dramatically, as do those on *Competitive in 2020 × Unified Republican Control*. Moreover, the addition of these race and COVI variables improves the R^2^ by well over 50%.
\newpage
\begin{landscape}
...
```{r mefs-state, echo = FALSE, message = F, warning = F, fig.width = 7.5, fig.height = 7, fig.cap = "\\label{fig:mefs-state}Restrictive Provisions Introduced and Passed, by Race and 2020 COVI", fig.pos = "!ht", fig.align = "center"}
print(readRDS("temp/mef_race_both.rds"))
```

```{r mefs-state-comp, echo = FALSE, message = F, warning = F, fig.width = 7.5, fig.height = 7, fig.cap = "\\label{fig:mefs-state-comf}Restrictive Provisions Introduced and Passed, by Competitiveness and Republican Control", fig.pos = "!ht", fig.align = "center"}
print(readRDS("temp/mef_part_both.rds"))
```

Figure \ref{fig:mefs-state} lays these relationships out starkly: less-white states where the COVI was already high were substantially more likely to introduce and pass restrictive legislation than A) whiter states with high COVIs, and B) less-white states with lower COVIs. Generally speaking, the whitest states did not consider restrictive legislation in 2021, regardless of their 2020 COVI. The same was true of less-white states with lower 2020 COVIs. Figure \ref{fig:mefs-state} indicates that the regressive legislation of 2021 was concentrated in a handful of states and provides very strong support for H1b and H2 (and, by extension, H1b). In the Supplementary Information, I show that these relationships are very different when we consider the introduction and passage of *expansive* voting bills. It is worth noting that these curves are *not meaningfully changed* in the second row of Figure \ref{fig:mefs-state} when I control for partisanship. In other words, the observed relationships cannot be explained by the fact that race and partisanship are closely tied.
