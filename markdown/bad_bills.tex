...
\begin{itemize}
\item
  States introduced and passed restrictive bills in 2021 in order to increase the security of their elections. Insofar as expansive voting laws are associated with (perceived) election insecurity, \textbf{H1a}: I expect that more restrictive voting provisions were introduced and passed in 2021 in states where the COVI was lower in 2020.
\item
  According to past literature, however, the COVI might capture more than just the permissiveness of the voting regime. As discussed above, \protect\hyperlink{ref-Bentele2013}{Bentele and O'Brien} (\protect\hyperlink{ref-Bentele2013}{2013}) and others argue that the difficulty of voting in a given state reflects white backlash and a response to racial threat. This theory would predict an entirely different legislative pattern in 2021 than H1a. If the 2020 COVI is indicative of how likely a state is to react to displays of nonwhite political power, I would expect \textbf{H1b}: states with higher COVIs introduced and passed more restrictive legislation in 2021 than states with lower COVIs.
\end{itemize}
...
In Table \ref{tab:state}, I present the results of tests on Hypotheses 1a, 1b, and 2. The coefficients on \emph{2020 COVI} provide tests for H1a and H1b. Because the regression has just 50 observations, I mark observations that are significant at just the 90\% confidence-level; while these are not statistically significant at the general threshold, they made indicate an underlying relationship.

Across the various specifications, the 2020 Cost of Voting Index is perhaps associated with an \emph{increase} in the number of restrictive provisions introduced (though the coefficient does not generally reach typical levels of significance in Models 1, 2, and 4). Put differently: states where it was already the hardest to vote may have made it \emph{even harder} to do so during the 2021 legislative session. This is provides support for H1b and undermines support for H1a. The fact that places where it was already hard to vote introduced and passed more restrictive legislation indicates that changes in voting legislation may bear little relationship to real concerns about election integrity.

In Models 1--4, the \emph{p}-value of the joint significance tests for the coefficients on the \emph{Nonhispanic White} terms is generally above 0.1, indicating a lack of a strong relationship between the racial composition of the state and its legislative behavior with respect to voting in 2021.

Models 5--8, however, indicate that the relationship between the white share of the state and the restrictive activity is \emph{highly} moderated by the 2020 COVI. Models 5--8 mirror models 1--4, with the addition of interactive terms between these variables. In each case, the addition of the interaction terms more than doubles the adjusted R\textsuperscript{2}; in the case of the models without other covariates, it is quadrupled. Once the interactions are included, the coefficients on the key independent variables are statistically significant at very high levels.
...
\leavevmode\hypertarget{ref-Gardner2021b}{}%
Gardner, Amy, Kate Rabinowitz, and Harry Stevens. 2021. {``How {GOP-backed} Voting Measures Could Create Hurdles for Tens of Millions of Voters.''} \emph{Washington Post}. https://www.washingtonpost.com/politics/interactive/2021/voting-restrictions-republicans-states/.
